...
def Classifier(DataPath, ProccessData=True, OneVone=True, OneVall=True, Train=True):
    '''
     Problem 3.1:
    ...
    testY = df_test['label'].to_numpy().reshape((1, 10000))
    testX = df_test[df_test.columns[1:]].to_numpy()
    df_train = pd.read_csv(DataPath)
    trainY = df_train['label'].to_numpy().reshape((1, 60000))
    trainX = df_train[df_train.columns[1:]].to_numpy()
...
if __name__ == '__main__':
    ...
    Classifier('./DataSet/fashion-mnist_test.csv', ProccessData=False, OneVall=False, Train=False)
